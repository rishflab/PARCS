...
void Chessboard::Init(const CvSize &size)
{
...
bool Chessboard::findCorners(const Mat &image, bool isTracking, vector<Point2f> &outCorners,
	int distortionThreshold)
{
	...
	Chessboard::GetThreshold(cvSize(w, h), patternSize, mParameters.maxPixels, mParameters.minPixels);
	...
	if (isTracking && preMarkers.size() == 4)
	{
		for (unsigned int i = 0; i<4; ++i)
		{
			...
			bool rst = Marker::detect(grayImage(r), 1, subMarker, mParameters);
			if (rst)
			{
	...
	else
	{
		...
		bool ret = Marker::detect(grayImage, 4, curMarkers, mParameters);
		if (!ret)
		{
			curMarkers.clear();
		}
	...
	Mat homo = Chessboard::findHomography4Pts(markerCenters, markerCentersInImage);
...
void Chessboard::GetThreshold(const CvSize &imageSize, const CvSize &boardSize, int &thMax, int &thMin)
{
...
Mat Chessboard::findHomography4Pts(const vector<Point2f> &src, const vector<Point2f> &dst)
{
	Mat h;
